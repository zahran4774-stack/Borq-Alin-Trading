...
type Product = { id: string; name_ar: string; name_en: string | null; sale_price: number };
...
export default function NewInvoiceForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error, data } = await supabase.rpc("create_sales_invoice", {
        ...
        <table className="w-full text-sm">
          <thead className="bg-navy/5 text-navy/70">
            <tr>
              ...
              <th className="px-3 py-2 text-start">{locale === "ar" ? "السعر" : "Price"}</th>
